...
function rememberForecast(id: string | null) {
  try { if (id) localStorage.setItem(savedForecastKey, id); else localStorage.removeItem(savedForecastKey) }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    getSites(controller.signal).then(async loaded => {
      ...
      try {
        ...
        if (forecast.forecast_id !== id || forecast.mode !== 'live' || ![24, 48].includes(forecast.horizon_hours)
  ...
  return <div className={`experience ${view === 'earth' ? 'earth-view' : 'turbine-view'} ${sheet ? 'has-sheet' : ''}`}>
    <main className="world-stage" aria-label="Исследование ветра и прогноза мощности" inert={sheet !== null || (assistantOpen && mobile)}>
      ...
      <footer className="world-footer"><span>ПОГОДА: OPEN-METEO · ВЕТЕР 10 М</span><a href="https://open-meteo.com/" target="_blank" rel="noreferrer">CC BY 4.0 ↗</a><button onClick={() => result ? openAnalytics('provenance') : setSheet('settings')}>О данных и сцене</button></footer>
    </main>
    ...
    <Sheet open={sheet === 'settings'} title="Ваш горизонт" subtitle="ПАРАМЕТРЫ ИССЛЕДОВАНИЯ" onClose={() => setSheet(null)} className="settings-sheet"><div className="settings-content"><p className="settings-lead">Выберите, на какой ветер смотреть.</p><div className="settings-weather"><span className="eyebrow">ИСТОЧНИК ПОГОДЫ</span><strong>Open-Meteo · ECMWF IFS</strong><p className="field-help">Текущий прогноз для координат турбины. Начало — следующий полный час; время выпуска определяет сервер. Синтетической замены при ошибке нет.</p>{result && <p className="field-help">{restored && <strong>Сохранённый прогноз. </strong>}Погода получена: {result.weather_provenance.retrieved_at ? localTime(result.weather_provenance.retrieved_at, result.timezone) : 'время не указано'}.{result.weather_provenance.weather_cache_hit === true && ' Использован погодный кеш.'}</p>}</div>
      ...
      <p className="data-note">Мощность нормализована. Точность прогноза на 24–48 часов пока не подтверждена. Операционный статус, реальные обороты и телеметрия SCADA недоступны. <a href="https://open-meteo.com/" target="_blank" rel="noreferrer">Open-Meteo · CC BY 4.0 ↗</a></p></div></Sheet>
