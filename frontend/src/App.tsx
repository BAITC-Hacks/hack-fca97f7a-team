  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="workspace"><section className="panel setup-panel"><div className="section-title"><span className="eyebrow">01 / CONFIGURE</span><h2>Select your forecast</h2><p>Choose a registered turbine and a replay origin.</p></div>
        ...
        <label className="field"><span>Turbine</span><select value={siteId} onChange={e => { invalidate(); setSiteId(e.target.value) }} disabled={!sites.length}><option value="">Select turbine</option>{sites.map(site => <option key={site.turbine_id} value={site.turbine_id}>{site.turbine_id} · {site.coordinate_status} location</option>)}</select></label>
        {siteError && <div className="error-banner">{siteError}</div>}
        <div className="field-grid"><label className="field"><span>Origin date</span><input type="date" min={FIRST_DATE} value={date} onChange={e => { invalidate(); setDate(e.target.value) }} /></label><label className="field"><span>Horizon</span><select value={horizon} onChange={e => { invalidate(); setHorizon(Number(e.target.value) as 24 | 48) }}><option value={24}>24 hours</option><option value={48}>48 hours</option></select></label></div>
        <label className="field"><span>Weather source</span><select value={mode} onChange={e => { invalidate(); setMode(e.target.value as WeatherMode) }}><option value="fixture">Synthetic fixture</option><option value="archive">Verified archive</option></select></label>
        <div className="origin-note">Forecast starts at 23:00 in {selectedSite?.timezone || LOCAL_ZONE} · {origin} UTC</div>
        <button className="primary-button" disabled={!siteId || loading} onClick={() => runForecast({ siteId, date, horizon, mode })}>{loading ? 'Calculating forecast…' : 'Predict generation'} <span>↗</span></button>
