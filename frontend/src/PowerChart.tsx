...
export default function PowerChart({ result, selectedIndex, onSelectIndex }: { result: ForecastResult, selectedIndex?: number, onSelectIndex?: (index: number) => void }) {
  ...
  return <div className="forecast-chart">
    ...
    {selectedIndex !== undefined && onSelectIndex && <label className="chart-time-control"><span>{localTime(result.hours[selectedIndex].valid_at, result.timezone)} · {percent(result.hours[selectedIndex].power_norm)}</span><input type="range" aria-label="Час на графике мощности" min={0} max={result.hours.length - 1} value={selectedIndex} onChange={event => onSelectIndex(Number(event.target.value))} /></label>}
