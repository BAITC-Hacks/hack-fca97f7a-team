...
def test_summary_rejects_invented_power(monkeypatch):
    explanation._CACHE.clear()
    monkeypatch.setenv("OPENAI_API_KEY", "unit-test-placeholder")
    client(monkeypatch, [response(text="Пиковая мощность составит 17 МВт.")])
    answer = summarize_forecast(forecast(), backend="llm")
    assert answer["backend"] == "template" and "0,82" in answer["text"]
    assert "не совпали с расчётом" in answer["warning"]
...
def test_invented_number_after_tool_uses_computed_fallback(monkeypatch):
    explanation._CACHE.clear()
    monkeypatch.setenv("OPENAI_API_KEY", "unit-test-placeholder")
    client(monkeypatch, [
        response({"type": "function_call", "name": "best_hours", "call_id": "best",
                  "arguments": json.dumps({"count": 4, "contiguous": True, "order": "best"})}),
        response(text="Лучший период: мощность 17 МВт."),
    ])
    answer = answer_question(forecast(), "Найди лучшие четыре часа подряд")
    assert answer["backend"] == "template"
    assert "0,896" in answer["text"] and "17 МВт" not in answer["text"]
    assert "не совпали с расчётом" in answer["warning"]
...
def test_grounding_allows_normalized_power_percent_but_not_invented_percent():
    tools = [{"data": {"mean_power_norm": 0.8, "mean_wind_speed_ms": 6.0}}]
    assert explanation._numbers_grounded("Средняя мощность 80%, ветер 6 м/с.", tools)
    assert not explanation._numbers_grounded("Средняя мощность 99%.", tools)
    assert not explanation._numbers_grounded("Мощность 0,8 МВт.", tools)
