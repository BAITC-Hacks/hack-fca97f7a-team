    ...
    for turbine_index, turbine_id in enumerate(("T1", "T2")):
        ...
        for origin in ORIGINS:
            for horizon in HORIZONS:
                ...
                csv = write_model_input(rows, turbine_id, origin, horizon)
                csv_path = artifact_dir() / "model_inputs" / csv["filename"]
                kwargs = dict(turbine_id=turbine_id, origin=origin, horizon_hours=horizon,
                              expected_sha256=csv["sha256"])
                first = model.predict_power_csv(bundle, csv_path, **kwargs)
                ...
                repeated = model.predict_power_csv(reloaded, csv_path, **kwargs)
