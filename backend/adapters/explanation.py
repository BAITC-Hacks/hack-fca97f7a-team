...
_LOCK = RLock()
_PROMPT_VERSION = "forecast-explanation-ru-v7-dialogue-grounded"
_MAX_MESSAGES = 10
_MAX_TOOL_CALLS = 4
_MAX_API_CALLS = 3
...
class _GroundingError(ValueError):
    """The prose added numerical claims absent from calculated forecast facts."""
...
def _explain(result: dict, backend: str, *, question: str | None = None) -> dict:
    ...
    instructions = (
        ...
        "Не переводи нормализованную мощность в проценты. "
    ...
    try:
        ...
        if not _numbers_grounded(answer.output_text, [{"text": fallback, "data": {"analysis": facts, "hours": result.get("hours", [])}}]):
            raise _GroundingError("ungrounded numeric summary")
        ...
        response["warning"] = "Числа в ответе ИИ не совпали с расчётом; показан расчётный ответ."
        return response
    except Exception:
...
def _numbers_grounded(spoken: str, tools: list[dict]) -> bool:
    """Reject numeric prose that cannot be found in a computed tool result."""
    if re.search(r"\d+(?:[.,]\d+)?\s*(?:МВт(?:[·*]ч)?|MW(?:h)?)\b", spoken, re.I):
        return False
    # UI displays normalized power as percentages; allow this exact unit conversion.
    powers = []
    def collect_power(value):
        if isinstance(value, dict):
            for key, item in value.items():
                if key.endswith("power_norm") and type(item) in (int, float) and 0 <= item <= 1:
                    powers.append(Decimal(str(item)) * 100)
                collect_power(item)
        elif isinstance(value, list):
            for item in value:
                collect_power(item)
    for tool in tools:
        collect_power(tool.get("data"))
    percent_pattern = r"(\d+(?:[.,]\d+)?)\s*%"
    for value in re.findall(percent_pattern, spoken):
        decimals = len(value.replace(",", ".").split(".")[1]) if "." in value or "," in value else 0
        tolerance = Decimal("0.5") / (Decimal(10) ** decimals)
        if not any(abs(Decimal(value.replace(",", ".")) - power) <= tolerance for power in powers):
            return False
    spoken = re.sub(percent_pattern, "", spoken)
    tokens = re.findall(r"(?<![\w])\d+(?:[.,]\d+)?(?![\w])", spoken)
    if not tokens:
        return True
    calculated = " ".join(json.dumps({"text": item.get("text"), "table": item.get("table"),
                                      "data": item.get("data")}, ensure_ascii=False, allow_nan=False)
                          for item in tools)
    allowed = set(re.findall(r"(?<![\w])\d+(?:[.,]\d+)?(?![\w])", calculated))
    allowed |= {value.replace(",", ".") for value in allowed}
    allowed |= {value.replace(".", ",") for value in allowed}
    def grounded(token: str) -> bool:
        ...
        try:
            ...
            if any(number == Decimal(value.replace(",", ".")) for value in allowed):
                return True
            if not ("," in token or "." in token):
                return False
            places = len(token.split(",")[-1].split(".")[-1])
            tolerance = Decimal(5) / (Decimal(10) ** (places + 1))
            return any(("," in value or "." in value)
                       and abs(number - Decimal(value.replace(",", "."))) <= tolerance
                       for value in allowed)
        except InvalidOperation:
            return False
    return all(grounded(token) for token in tokens)
...
def _question_instructions() -> str:
    return (
        ...
        "Не переводите её в проценты или МВт. "
...
def answer_question(result: dict, question: str, backend: str = "llm", *,
    ...
    try:
        ...
        try:
            for _ in range(_MAX_API_CALLS):
                ...
                if not calls:
                    ...
                    if tool_results and not _numbers_grounded(spoken, tool_results):
                        raise _GroundingError("ungrounded numeric response")
        ...
        fallback["warning"] = "Числа в ответе ИИ не совпали с расчётом; показан расчётный ответ."
        return fallback
    except Exception:
