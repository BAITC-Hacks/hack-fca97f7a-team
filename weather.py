...
def _save_raw(raw, digest):
    ...
    try:
        ...
        if target.exists():
            ...
            # Publish exclusively; never overwrite a concurrent writer's artifact.
            try:
...
def clear_live_weather_cache(turbine_id: str | None = None) -> None:
    """Discard cached live payloads for one turbine or all turbines."""
...
def _live_weather(site: dict, origin: str, horizon_hours: int) -> dict:
    ...
    with _LIVE_CACHE_LOCK:
        ...
        if entry is not None:
            age = utc_now() - utc_time(entry["retrieved_at"])
            if (time.monotonic() - entry["cached_at"] >= _LIVE_CACHE_TTL_SECONDS
                    or not timedelta(0) <= age < timedelta(minutes=5)
                    or utc_time(entry["retrieved_at"]) > utc_time(origin)):
                del _LIVE_CACHE[key]
                entry = None
        if entry is not None:
    ...
    if cache_hit:
        raw, fetched_at = entry["raw"], entry["retrieved_at"]
        try:
            data = json.loads(raw)
            rows, grid_lat, grid_lon, _ = _parse_forecast(data, origin, horizon_hours)
        except (ValueError, ForecastError):
            cache_hit = False
    if not cache_hit:
        try:
            ...
            if response.status_code != 200:
                raise _unavailable("Погодный API недоступен; повторите запрос позже.")
            if utc_time(fetched_at) > utc_time(origin):
                raise ForecastError("LIVE_ORIGIN_CHANGED", "Получение погоды пересекло начало прогноза; прогноз будет обновлён.")
            raw = response.content
            data = json.loads(raw)
        except httpx.TimeoutException as exc:
            raise _unavailable("Погодный API не ответил вовремя; повторите запрос позже.") from exc
    ...
    if not cache_hit:
        with _LIVE_CACHE_LOCK:
            _LIVE_CACHE[key] = {"turbine_id": site["turbine_id"], "raw": raw,
                                "retrieved_at": fetched_at, "cached_at": time.monotonic()}
            _LIVE_CACHE.move_to_end(key)
    ...
    return {"manifest": {"turbine_id": site["turbine_id"], "run_id": f"live-best-match/{digest}",
            ...
            "retrieved_at": fetched_at, "weather_cache_hit": cache_hit,
            "availability_basis": "response_received", "provenance_status": "live",
            "raw_sha256": digest, "interpolation": "none", "wind_height_m": 10,
            "temperature_height_m": 2, "weather_model": MODEL,
            "wind_height_status": "provider_feature_not_sensor_measurement",
            "grid_latitude": grid_lat, "grid_longitude": grid_lon}, "rows": rows}
